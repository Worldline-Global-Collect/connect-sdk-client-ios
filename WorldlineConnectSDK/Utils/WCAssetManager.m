...
@implementation WCAssetManager

- (instancetype)init
{
    self = [super init];
    if (self != nil) {
        NSString *sdkBundlePath = [[NSBundle mainBundle] pathForResource:@"WorldlineConnectSDK" ofType:@"bundle"];
         ...
         */
        if ([StandardUserDefaults boolForKey:kWCImageMappingInitialized] == NO) {
            ...
            [StandardUserDefaults setObject:imageMapping forKey:kWCImageMapping];
            [StandardUserDefaults setBool:YES forKey:kWCImageMappingInitialized];
...
- (NSString *)logoIdentifierWithPaymentItem:(NSObject<WCPaymentItem> *)paymentItem {
...
{
    for (NSObject<WCPaymentItem> *paymentItem in paymentItems) {
...
- (void)initializeImagesForPaymentItem:(NSObject<WCPaymentItem> *)paymentItem {
    paymentItem.displayHints.logoImage = [self logoImageForPaymentItem:paymentItem.identifier];
    for (WCPaymentProductField *field in paymentItem.fields.paymentProductFields) {
...
{
    NSMutableDictionary *imageMapping = [[[NSUserDefaults standardUserDefaults] objectForKey:kWCImageMapping] mutableCopy];
    for (NSObject<WCPaymentItem> *paymentItem in paymentItems) {
...
- (void)updateImagesForPaymentItem:(NSObject<WCPaymentItem> *)paymentItem baseURL:(NSString *)baseURL
{
    NSMutableDictionary *imageMapping = [[[NSUserDefaults standardUserDefaults] objectForKey:kWCImageMapping] mutableCopy];
    for (WCPaymentProductField *field in paymentItem.fields.paymentProductFields) {
        if (field.displayHints.tooltip.imagePath != nil) {
