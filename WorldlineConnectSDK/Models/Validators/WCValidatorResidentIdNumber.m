...
#import "WCResidentIdNumberError.h"

@interface WCValidatorResidentIdNumber ()

@end

@implementation WCValidatorResidentIdNumber
...
- (void)validate:(NSString *)value forPaymentRequest:(WCPaymentRequest *)request
{
    ...
    if (value.length == 15) {
        ...
        if (value.intValue == 0) {
            [self.errors addObject:[[WCResidentIdNumberError alloc] init]];
            return;
        }
