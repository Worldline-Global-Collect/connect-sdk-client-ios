//
//  WCValidatorFixedListTestCase.m
//  WorldlineConnectSDK
//
//  Created for Worldline Global Collect on 15/12/2016.
//  Copyright © 2017 Worldline Global Collect. All rights reserved.
//

#import <XCTest/XCTest.h>
#import  "WCValidatorFixedList.h"

@interface WCValidatorFixedListTestCase : XCTestCase

@property (strong, nonatomic) WCValidatorFixedList *validator;

@end

@implementation WCValidatorFixedListTestCase

- (void)setUp
{
    [super setUp];
    self.validator = [[WCValidatorFixedList alloc] initWithAllowedValues:@[@"1"]];
}

- (void)tearDown
{
    [super tearDown];
}

- (void)testValidateCorrect
{
    [self.validator validate:@"1" forPaymentRequest:nil];
    XCTAssertTrue(self.validator.errors.count == 0, @"Valid value considered invalid");
}

- (void)testValidateIncorrect
{
    [self.validator validate:@"X" forPaymentRequest:nil];
    XCTAssertTrue(self.validator.errors.count != 0, @"Invalid value considered valid");
}

@end
